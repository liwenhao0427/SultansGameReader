...
- [ ] 1. Electron scaffolding, Vite + React setup, and IPC communication
  - [ ] 1.1 Initialize project with package.json, Vite config, and Electron entry point
  ...
  - [ ] 1.2 Create Electron main process with IPC handlers
    ...
    - IPC channels: `config:setGameDir`, `config:rebuildCache`, `config:clearCache`, `config:readCache`, `config:listCache`, `config:buildIndex`, `asset:setCliPath`, `asset:extract`, `asset:resolveImage`, `file:readRaw`, `settings:get`, `settings:set`
    ...
    - `config:rebuildCache`: instantiate CacheManager, call `scanAll()` with progress callback, send progress via IPC event
    ...
    - `config:buildIndex`: scan all cache subdirectories and single files, return flat array of `{ id, type, name, text }`
    - `asset:resolveImage`: implement image resolution chain (Sprite .png → Sprite .png.png → Texture2D .png → Texture2D .png.png → null)
  ...
  - [ ] 1.3 Create preload script exposing IPC API to renderer
  ...
  - [ ]* 1.4 Write unit tests for IPC handlers
    - Test `config:setGameDir` validates path correctly (valid and invalid paths)
    - Test `asset:resolveImage` fallback chain (all 4 paths + null)
    - Test `config:buildIndex` returns correct structure
...
- [ ] 3. Settings page
  - [ ] 3.1 Create SettingsPage component with directory/path selectors
  ...
  - [ ] 3.2 Implement cache and resource management buttons
  ...
  - [ ] 3.3 Add routing between settings page and main layout
...
- [ ] 5. Search panel and index construction
  - [ ] 5.1 Create useConfigStore with search index
    - Create `sultan-reader/src/stores/useConfigStore.js` using Zustand
    - State: `searchIndex` (array of `{ id, type, name, text }`), `cardsMap` (Map of card id → card data for condition parser), `counterRegistry` (Map of counter id → { id, comment, value }), `isLoaded` flag
    - Action: `buildIndex()` — calls `electronAPI.configBuildIndex()`, populates searchIndex; also loads cards.json into cardsMap
    - _Requirements: 4.1, 17.2_

  - [ ] 5.2 Create SearchPanel component
    - Create `sultan-reader/src/components/SearchPanel.jsx`
    - Text input for keyword search, fuzzy matching against id, name, text fields in searchIndex
  ...
  - [ ] 5.3 Implement drag-from-search to canvas
  ...
  - [ ]* 5.4 Write unit tests for search filtering
    - Test fuzzy matching logic with various query strings
    - Test type filter combinations
    - _Requirements: 4.2, 4.3_

- [ ] 6. XYFlow canvas with node rendering and edge styling
  - [ ] 6.1 Create useCanvasStore for canvas state management
  ...
  - [ ] 6.2 Create custom node components for each type
  ...
  - [ ] 6.3 Create Canvas component with @xyflow/react
  ...
  - [ ] 6.4 Create edgeExtractor service
  ...
  - [ ] 6.5 Implement auto-expand logic and edge styling
    - When a node is added: call edgeExtractor, count associations
    - If associations ≤ 10: auto-load all associated nodes via IPC, add them to canvas with edges
    - If associations > 10: add node in collapsed state, show expansion controls in DetailPanel
  ...
  - [ ] 6.6 Implement automatic layout with dagre
    - Configure dagre for automatic node positioning
  ...
  - [ ]* 6.7 Write unit tests for edgeExtractor
...
- [ ] 8. Condition parser service
  - [ ] 8.1 Create conditionParser service
  ...
  - [ ]* 8.2 Write unit tests for conditionParser
...
- [ ] 9. Image resolver service
  - [ ] 9.1 Create imageResolver service
    - Create `sultan-reader/src/services/imageResolver.js`
    - `resolveImage(pic)` → calls `electronAPI.assetResolveImage(pic)`, returns file path or null
    - Actual resolution logic (4-step fallback chain) is in main process IPC handler (task 1.2)
    - Renderer-side wrapper handles null → placeholder image display
    - _Requirements: 12.1, 12.2, 12.3, 12.4_

- [ ] 10. Detail panels for all node types
  - [ ] 10.1 Create DetailPanel container with type dispatch
  ...
  - [ ] 10.2 Create EventDetail component
  ...
  - [ ] 10.3 Create RiteDetail component
  ...
  - [ ] 10.4 Create AfterStoryDetail component
  ...
  - [ ] 10.5 Create CardDetail, LootDetail, OverDetail, UpgradeDetail, DTDetail components
  ...
  - [ ]* 10.6 Write unit tests for detail panel rendering
...
- [ ] 13. Counter management and player state simulation
  - [ ] 13.1 Build counter registry from cached data
  ...
  - [ ] 13.2 Create usePlayerStore for simulation state
  ...
  - [ ] 13.3 Create CounterPanel component
  ...
  - [ ] 13.4 Implement condition evaluation and branch highlighting
  ...
  - [ ]* 13.5 Write unit tests for conditionEvaluator
...
- [ ] 15. AssetStudio integration
  - [ ] 15.1 Implement asset extraction IPC handler
    - In `electron/main.js`, implement `asset:extract` handler
    - Sprite extraction: `AssetStudio.CLI.exe "<gamePath>\Sultan's Game_Data" "<resourceDir>" --game Normal --types Sprite:Both --group_assets ByType --image_format Png`
    - Texture2D extraction: `AssetStudio.CLI.exe "<gamePath>\Sultan's Game_Data" "<resourceDir>" --game Normal --types Texture2D --group_assets ByType --image_format Png`
    - Use `child_process.spawn`, capture stdout/stderr, send progress events to renderer
    - Validate CLI path exists before execution, return error if not found
    - Handle non-zero exit codes, return error output
    - _Requirements: 13.1, 13.2, 13.3, 13.4, 13.5_

  - [ ] 15.2 Update SettingsPage with extraction progress UI
...
- [ ] 16. Raw file comparison view
  - [ ] 16.1 Create RawFileView component
...
- [ ] 17. Performance optimizations for large files
  - [ ] 17.1 Implement virtualized rendering for large detail content
